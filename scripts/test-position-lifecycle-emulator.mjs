...
try {
  ...
  await denied(setDoc(doc(db, "positions", "RM-01"), positionData));
  await denied(deleteDoc(doc(db, "counters", "positionsV2")));
  await denied(setDoc(doc(db, "counters", "positionsV2"), { next: 1 }));
  ...
  assert.equal(recreated.id, "vacancy-v2-4");
  await denied(setDoc(doc(db, "applications", "after-delete"), { positionId, stage: "applied" }));
  console.log("PASS: concurrent identities, immutable counter, >500 dependent records, rejection cleanup, deletion locks, employee snapshots, isolated cleanup, retry, and same-title recreation.");
} finally {
