...
it("shows the empty-state copy for both sections and calls onRemoveDay on click (weekend only)", () => {
  ...
  expect(rendered).toContain("No blocked times.");
  buttons(tree).find((b) => text(b) === "Remove this day").props.onClick();
  expect(onRemoveDay).toHaveBeenCalled();
  tree.unmount();
});

it("a removed day (enabled:false) shows the muted message + Restore button instead of the two sections", () => {
  const onRestoreDay = vi.fn();
  const tree = create(<DayPanel dayKey="sun" day={{ enabled: false, available: [], blocked: [] }} validation={noValidation} onAddRow={vi.fn()} onChangeRow={vi.fn()} onRemoveRow={vi.fn()} onRemoveDay={vi.fn()} onRestoreDay={onRestoreDay} />);
...
it("surfaces a row error and a row warning at the right index", () => {
  const day = { enabled: true, available: [{ start: "09:00", end: "12:00" }], blocked: [{ start: "14:00", end: "15:00" }] };
  const validation = { errors: [], warnings: [{ list: "blocked", index: 0, message: "This blocked time is outside your available hours" }] };
  ...
  expect(JSON.stringify(tree.toJSON())).toContain("This blocked time is outside your available hours");
  tree.unmount();
});
