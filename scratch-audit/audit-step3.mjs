...
const run = async () => {
  ...
  for (const [name, url] of staffPages) {
    ...
    await auditPage(staff, name, url, [...ZOOM_WIDTHS, ...NARROW_WIDTHS], results, "step3-staff");
  }
  ...
  writeFileSync("scratch-audit/results-step3.json", JSON.stringify(results, null, 2));
