...
const run = async () => {
  ...
  for (const [name, url] of staffPages) {
    ...
    await auditPage(staff, name, url, [...ZOOM_WIDTHS, ...NARROW_WIDTHS], results, "final-staff");
  }
  ...
  for (const [name, url] of candPages) {
    console.log("Auditing (candidate):", name);
    await auditPage(cand, name, url, [...ZOOM_WIDTHS, ...NARROW_WIDTHS], results, "final-cand");
  }

  await cand.close();
  await browser.close();

  writeFileSync("scratch-audit/results-final.json", JSON.stringify(results, null, 2));
