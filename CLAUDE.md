...
- **TEMPORARY DEVIATION (2026-09-12):** Firebase Storage now requires the project to be on the Blaze plan just to provision a bucket at all — discovered live when a real application hung indefinitely on submit, because `uploadCv()` was targeting a bucket that had never been created (`firebase deploy --only storage` fails outright with "Firebase Storage has not been set up on project"). Blaze is pay-as-you-go with the same free monthly allowance as the free plan — realistic cost for this project is $0 — but it requires a card on file, which the team does not want to add. Until Storage is provisioned, this hard rule does not hold: every CV goes through the legacy base64-in-Firestore path instead (`fileToDataUrl()` in `src/lib/file.js`, already existed for pre-Storage candidates), and `MAX_CV_BYTES` is lowered from 5MB to 500KB to stay under Firestore's ~1MB per-document limit once the rest of the candidate identity document (education, experience, skills, `cvExtractedText`) is accounted for. `storage.rules` still states 5MB because that's the correct value for when Storage resumes — not the value actually enforced right now. Revert (`uploadCv()` in `ApplyModal.jsx`'s `submit()`, `MAX_CV_BYTES` in `file.js`) the moment Storage is provisioned; nothing downstream needs to change since both paths already produce a URL string.
...
**First production data point (one real CV, scored against a live vacancy, 2026-09-11):** `instrumentation.layers = {normalisation: 5, embedding: 0, none: 3}`, `borderline: 0`. Layer 3 fired — a Workers AI call was made for "Container orchestration" (no literal match in the CV, which states "Kubernetes") and for "Cloud infrastructure" (the CV states "AWS"/"Terraform") — and credited neither: `Container orchestration`/`Kubernetes` measures 0.7276 against the calibrated 0.8804 threshold, consistent with the WS6.1 calibration finding, not a contradiction of it. **`borderline: 0` is itself a finding, and the direct answer to the obvious challenge that embeddings aren't run-to-run reproducible (6.4): on this run, no match sat close enough to the threshold for that instability to have changed anything — embedding non-determinism could not have affected this score, because nothing was close enough to the boundary to flip.** One CV is one data point, not a firing rate — the decision stays open pending the real firing rate and BORDERLINE count across the full WS6.3 fixture set.
...
**Implementation status (2026-09-11):** only the fallback tier — steps 1–4 (section-priority detection and pruning) are not built; `truncationStrategy` is currently always `HEAD_TAIL_FALLBACK` when truncation fires, never `SECTION_PRIORITY`, and `sectionsPruned` is not yet populated. What **is** live and tested: a long CV is never rejected for length (the previous behavior — a hard block above 20,000 chars, plus an independent, unrelated blind 6,000-char head-slice sent to the model regardless of length — was a real bug, not a deferred feature, and is fixed); truncation runs once, authoritatively, in the Worker (`truncateForModel()`, shared by validate-cv and parse-cv) on a 15,000-char budget; `truncationApplied`/`truncationStrategy` are returned by both endpoints and surfaced to HR on the candidate detail view. Section-priority truncation is deferred — deliberately, to keep Tier 1 evidence (WS6) and WS8 on schedule — not silently dropped; this note is the record of that decision. The Red/Yellow/Green band table above is likewise the target design, not the current routing: today there are exactly two outcomes before the Worker — reject under `MIN_CHARS` (Red), or proceed (Green/Yellow collapsed into one path) — there is no binary-corruption-marker detection and no separate missing-section-markers escalation trigger. Revisit if 6.6's heuristic catch rate shows the two-outcome gate is letting through cases the three-band design would have caught.
...
**A third finding, not variance but fidelity — caught by testing end-to-end, not assumed:** the WS4 extraction schema originally asked for one combined `degree` string ("BSc (Hons) Computer Science"). Run against a real CV, extraction returned `"BSc (Hons)"` and silently dropped the field of study. The candidate — who genuinely holds a Computer Science degree — scored **0/25 on qualifications, overallScore 52**, purely because the parser lost a field, not because of anything about the candidate. Fixed by splitting the schema into independently-required `awardType`/`field` slots and deriving degree level deterministically in JS rather than trusting the model with it (5.10's "model extracts facts, code computes" split, applied to the one place it wasn't yet). Re-run against the identical CV: qualifications 25/25, **overallScore 77** — a 25-point swing from a single schema change. Recorded here because a 6.4 stability claim is worthless if the pipeline it's measuring is quietly wrong in the first place; this is what running a real CV through the real, deployed chain is for, and it found a real bug before the WS6.3 fixture set would have.
...
**Consequence for WHO can write a score, found live 2026-09-12.** `applicationScores` is staff-*write*-only too, not just staff-read-only — the same rule line does both, and it has to: if a candidate's own session could write their own score, they could fabricate it (write `overallScore: 100` and nothing would ever check it against a real CV), which defeats the entire point of a server-computed, trustworthy score. This directly ruled out having a candidate's own apply-time submission compute and write its own score — a design this project briefly had — because every self-application to a position with `requirements` set failed outright with `permission-denied` on the `applicationScores` write, discovered when a real candidate hit it. **Scoring is staff-triggered, not candidate-triggered:** an application is created unscored, and HR's "Re-score all" on the shortlist (`rescoreVacancy()`, staff-authenticated) is what populates `applicationScores` — matching how the shortlist already has to handle "not scored — retry" per 10.1 regardless. A server-side write path (the Worker itself writing the score with its own elevated credentials, bypassing the candidate's session entirely) would restore true score-on-submit and is the correct long-term fix, but needs a Firestore-writing credential inside a Cloudflare Function — real, separate infrastructure work, not attempted under the pressure of an active outage. Documented here so nobody "simplifies" this back to a candidate-side write later — see the paragraph above for why that specific mistake is easy to make twice.

**Identity and score records outlive their application — deliberately, confirmed live 2026-09-12.** `candidates/{id}` and `applicationScores/{id}` both carry `allow delete: if false`, and both were checked against the commits that introduced them rather than assumed: `candidates`' delete rule was written the same day as the WS1 identity/application split, with its own stated reason ("identity persists regardless of how many applications come and go, or whether the person is eventually hired") — a continuation of the pre-existing "a REJECTED candidate can never be deleted, the talent pool is permanent" principle from the original rules file. `applicationScores`' delete rule was written the same day the collection was split out for R3, with its own stated reason ("score history persists — never silently erased") — the write-side twin of the anti-forgery reasoning two paragraphs up: if a score can be quietly deleted, "the machine can be checked" (5.8) stops being true. Both are deliberate audit-integrity decisions, not unexamined defaults, and the record of that reasoning already existed in the rules file's own comments before this note; this paragraph is what makes it findable from the spec.

**Consequence, hit live the same day:** a non-CV application (a project-planning PDF uploaded as a "CV," predating WS3's validation gate — the exact failure named in §5's mistake #9) was found on BD-01's real shortlist and its `applications` doc was deleted (staff may delete a non-`rejected` application they own the stage of). Its `candidates` identity and `applicationScores` doc could not be deleted through the app, by the design above, and now exist as orphans — a `candidates/{email}` doc with no application referencing it, and an `applicationScores/{id}` doc whose `positionId` still points at a real vacancy but whose `applications/{id}` no longer exists. **Accept this as the price of the two guarantees above, not as a bug to route around.** Cleanup of genuinely-contaminated data (never of a real candidate's identity) is a deliberate, out-of-band Admin SDK action, logged in a commit — never a client-side delete path, which would reopen exactly the hole these rules close.

**"Renders nowhere" was verified, not assumed, when this was found.** Every candidate-shaped view in the app (`CandidatesTable`, `ShortlistPanel`, `PositionDetail`) reads from `src/data/store.js`'s `candidates` array, which is built by mapping over **live `/applications` docs** and joining each to its identity (`liveApplications = [...appById.values()].map(app => joinFlat(identityById.get(app.personId), app))`) — never by iterating `/candidates` or `/applicationScores` directly. An identity or score with no matching application is structurally excluded from every one of these views, not just absent by convention. One real, narrow exception found while checking this: `updatePosition()`'s stale-marking query (5.2) matches `applicationScores` by `positionId` alone, so editing a vacancy's requirements will still write `stale: true` to an orphaned score left over from a deleted application on that same vacancy. Harmless — nothing ever reads that document again — but real, and recorded here rather than left as an unverified "should be fine."
