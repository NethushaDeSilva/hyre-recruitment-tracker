...
const run = async () => {
  ...
  for (const [name, url] of staffPages) {
    ...
    await auditPage(staff, name, url, [...ZOOM_WIDTHS, ...NARROW_WIDTHS], results, "staff");
  }
  ...
  for (const [name, url] of candPages) {
    ...
    await auditPage(cand, name, url, [...ZOOM_WIDTHS, ...NARROW_WIDTHS], results, "cand");
  }

  await cand.close();
  await browser.close();

  writeFileSync("scratch-audit/results.json", JSON.stringify(results, null, 2));
